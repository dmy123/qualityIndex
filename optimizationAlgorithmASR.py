    ...
    while (k <= iterations):
        ...
        if (k == M(i, b)):
            ...
            if (i == 1):
                ...
                paint[best_solution_change_count] = [simulation_budget, -function(new_sample,model)[0], new_sample_value]
                ...
                while (count > 0):
                    ...
                    add += -function(new_sample,model)[0]
                    count = count - 1
            ...
            for t in range(accepted_count):
                if (estimated_value[t][1] < rise_rate_K(i, c, Ck)):
                    ...
                    while (additional_ob_times > 0):
                        ...
                        additional_observe = additional_observe - function(accepted_set[t][:dimension],model)[0]
                    ...
                    if (accepted_set[t][dimension] > best_solution[dimension]):
                        ...
                        paint[best_solution_change_count] = [simulation_budget, -function(accepted_set[t][:dimension],model)[0],
            ...
            resample_value,paint_noise[k-1] = function(accepted_set[resample_loc][:dimension],model)
            ...
            if (accepted_set[resample_loc][dimension] > accepted_set[best_solution_loc][dimension]):
                ...
                paint[best_solution_change_count] = [simulation_budget,
                                                     -function(accepted_set[resample_loc][:dimension],model)[0],
                                                     best_solution[dimension]]
...
def function(X,model):
    ...
    h.append(h_q(X[3]))
    # print(X)
    # return lossFunction(h, 0.53, 1530, 1, model)  # 不加速，不加噪音
    add_noise = max(0.4, min(0.6, 0.53 + noise(0, 0.1)))
    add_noise = round(add_noise, 2)
    return lossFunction(h, add_noise, 1530, 1, model)-2.7,add_noise   # 不加速，加噪音
    # return lossFunction(h,0.53,1530,0,model)      # 加速，不加噪音
    # return lossFunction(h, add_noise, 1530, 0, model)  # 加速，加噪音
